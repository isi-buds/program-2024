...
```{r}
add_two_numbers <- function(number1=1, number2=2){
...
my_exp_sample <- rexp_isi_buds(10000, exp_rate = 0.1)
```
...
data.frame("my_exp_sample" = my_exp_sample) %>% # Need data frame for ggplot()
  ...
  geom_histogram() # Try changing the number of bins by setting bins = 10
```

Let's compare our simulations with simulations produced by the R function `rexp()`

```{r}
my_exp_sample2 <- rexp(10000, 0.1)
