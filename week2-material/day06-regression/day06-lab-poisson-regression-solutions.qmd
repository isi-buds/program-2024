---
title: "Day 6 Lab: Poisson Regression Solutions"
format: 
  html:
    toc: true
    toc-expand: true
...
```{r}
shark_data <- readxl::read_xls("floridashark.xls")
```
...
If the following model were fit $$\log(Attacks_i) = \beta_0 + \beta_1 Year_i,$$ the intercept would not have a meaningful interpretation.
Instead let's make it have a meaningful interpretation by adjusting for the baseline year $$\log(Attacks_i) = \beta_0 + \beta_1 I(Year_i - 1946).$$
...
Linear model $$E[log(Attack_i_i)] = \beta_0 + \beta_1 I(Year - 1964)$$
